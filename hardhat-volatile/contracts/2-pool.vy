...
MIN_A: constant(uint256) = 2**2 * A_MULTIPLIER / 10
MAX_A: constant(uint256) = 2**2 * A_MULTIPLIER * 100000

# This must be changed for different 2
# For example:
# 2 = 3 -> 1  (10**18 -> 10**18)
# 2 = 4 -> 10**8  (10**18 -> 10**10)
# PRICE_PRECISION_MUL: constant(uint256) = 1
PRECISIONS: immutable(uint256[2])
...
@internal
@pure
def geometric_mean(unsorted_x: uint256[2], sort: bool) -> uint256:
    """
    (x[0] * x[1] * ...) ** (1/N)
    """
    x: uint256[2] = unsorted_x
    if sort and x[0] < x[1]:
...
@internal
@view
def newton_D(ANN: uint256, gamma: uint256, x_unsorted: uint256[2]) -> uint256:
    """
    ...
    D: uint256 = 2 * self.geometric_mean(x, False)
    S: uint256 = x[0] + x[1]
    ...
    for i in range(255):
        ...
        K0: uint256 = (10**18 * 2**2) * x[0] / D * x[1] / D
        ...
        # 2*N*K0 / _g1k0
        mul2: uint256 = (2 * 10**18) * 2 * K0 / _g1k0

        neg_fprime: uint256 = (S + S * mul2 / 10**18) + mul1 * 2 / K0 - mul2 * D / 10**18
...
@internal
@pure
def newton_y(ANN: uint256, gamma: uint256, x: uint256[2], D: uint256, i: uint256) -> uint256:
    """
    Calculating x[i] given other balances x[0..2-1] and invariant D
    ANN = A * N**N
    """
    # Safety checks
    assert ANN > MIN_A - 1 and ANN < MAX_A + 1  # dev: unsafe values A
    ...
    x_j: uint256 = x[1 - i]
    y: uint256 = D**2 / (x_j * 2**2)
    K0_i: uint256 = (10**18 * 2) * x_j / D
    # S_i = x_j

    # frac = x_j * 1e18 / D => frac = K0_i / 2
    assert (K0_i > 10**16*2 - 1) and (K0_i < 10**20*2 + 1)  # dev: unsafe values x[i]

    # x_sorted: uint256[2] = x
    # x_sorted[i] = 0
...
def get_xcp(D: uint256) -> uint256:
    x: uint256[2] = [D / 2, D * PRECISION / (self.price_scale * 2)]
...
@internal
def tweak_price(A_gamma: uint256[2],_xp: uint256[2], p_i: uint256, new_D: uint256):
    ...
    xp: uint256[2] = [D_unadjusted / 2, D_unadjusted * PRECISION / (2 * price_scale)]
...
def _exchange(sender: address, mvalue: uint256, i: uint256, j: uint256, dx: uint256, min_dy: uint256, use_eth: bool) -> uint256:
    ...
    assert i < 2  # dev: coin index out of range
    assert j < 2  # dev: coin index out of range
...
@view
@internal
def _calc_token_fee(amounts: uint256[2], xp: uint256[2]) -> uint256:
    # fee = sum(amounts_i - avg(amounts)) * fee' / sum(amounts)
    fee: uint256 = self._fee(xp) * 2 / (4 * (2-1))
    S: uint256 = 0
    for _x in amounts:
        S += _x
    avg: uint256 = S / 2
...
@external
@nonreentrant('lock')
def add_liquidity(amounts: uint256[2], min_mint_amount: uint256, use_eth: bool = False) -> uint256:
    assert not self.is_killed  # dev: the pool is killed
    ...
    xp: uint256[2] = self.balances
    amountsp: uint256[2] = empty(uint256[2])
    xx: uint256[2] = empty(uint256[2])
    d_token: uint256 = 0
...
@external
@nonreentrant('lock')
def remove_liquidity(_amount: uint256, min_amounts: uint256[2], use_eth: bool = False):
    """
...
@view
@external
def calc_token_amount(amounts: uint256[2]) -> uint256:
    token_supply: uint256 = CurveToken(token).totalSupply()
    price_scale: uint256 = self.price_scale * PRECISIONS[1]
    A_gamma: uint256[2] = self._A_gamma()
    xp: uint256[2] = self.xp()
    amountsp: uint256[2] = [
...
def _calc_withdraw_one_coin(A_gamma: uint256[2], token_amount: uint256, i: uint256, update_D: bool,
                            calc_price: bool) -> (uint256, uint256, uint256, uint256[2]):
    token_supply: uint256 = CurveToken(token).totalSupply()
    assert token_amount <= token_supply  # dev: token amount more than supply
    assert i < 2

    xx: uint256[2] = self.balances
    D0: uint256 = 0

    price_scale_i: uint256 = self.price_scale * PRECISIONS[1]
    xp: uint256[2] = [xx[0] * PRECISIONS[0], xx[1] * price_scale_i / PRECISION]
    if i == 0:
