...
module clamm::stable_tuple_5pool_new_tests {
  ...
  fun sets_initial_state_correctly() {
    ...
    next_tx(test, alice);
    {
      ...
      let index = interest_clamm_stable::coin_index<FRAX, LP_COIN>(&mut pool);
      let balance = interest_clamm_stable::coin_balance<FRAX, LP_COIN>(&mut pool);

      assert_eq(index, 3);
      assert_eq(balance, 45000 * FRAX_DECIMALS_SCALAR);

      let index = interest_clamm_stable::coin_index<TRUE_USD, LP_COIN>(&mut pool);
      let balance = interest_clamm_stable::coin_balance<TRUE_USD, LP_COIN>(&mut pool);
