  ...
  fun add_liquidity() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      let lp_coin = interest_amm_stable::add_liquidity_5_pool<DAI, USDC, USDT, FRAX, TRUE_USD, LP_COIN>(
        &mut pool,
        &c,
  ...
  fun add_liquidity_coins_wrong_order() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      burn(interest_amm_stable::add_liquidity_5_pool<FRAX, USDC, DAI, USDT, TRUE_USD, LP_COIN>(
        &mut pool,
        &c,
        mint<FRAX>(130, FRAX_DECIMALS, ctx(test)),
        mint<USDC>(110, USDC_DECIMALS, ctx(test)),
