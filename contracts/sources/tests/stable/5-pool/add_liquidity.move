...
#[test_only]
module clamm::stable_tuple_5pool_add_liquidity_tests {
    ...
    next_tx(test, alice);
    {
      ...
      let expected_value = interest_clamm_stable::quote_add_liquidity<LP_COIN>(
        ...
        vector[
          ...
          add_decimals(140, TRUE_USD_DECIMALS)
        ]
      );
      ...
      let new_balances = vector[
        ...
        balances[4] + normalize_amount(140)
      ];
  ...
  fun add_liquidity_coins_wrong_order() {
   let mut scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      burn(interest_clamm_stable::add_liquidity_5_pool<FRAX, USDC, DAI, USDT, TRUE_USD, LP_COIN>(
        &mut pool,
        &c,
        mint<FRAX>(130, FRAX_DECIMALS, ctx(test)),
        mint<USDC>(110, USDC_DECIMALS, ctx(test)),
