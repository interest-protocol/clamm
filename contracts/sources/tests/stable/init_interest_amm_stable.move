...
module clamm::init_interest_amm_stable {
  ...
  public fun setup_2pool(test: &mut Scenario, usdc_amount: u64, usdt_amount: u64) {
    let (alice, _) = people();
    ...
    next_tx(test, alice);
    {
      ...
      sim::set_state(
        ...
        initial_a, 
        2, 
        vector[normalize_amount((usdc_amount as u256)), normalize_amount((usdt_amount as u256))],
        1
      );
  ...
  public fun setup_3pool(test: &mut Scenario, dai_amount: u64, usdc_amount: u64, usdt_amount: u64) {
    ...
    next_tx(test, alice);
    {
      ...
      let (mut pool, lp_coin) = interest_clamm_stable::new_3_pool(
        &c,
        &coin_decimals,
  ...
  public fun setup_4pool(
    ...
    next_tx(test, alice);
    {
      ...
      let (pool, lp_coin) = interest_clamm_stable::new_4_pool(
        &c,
        &coin_decimals,
        mint<DAI>(dai_amount, DAI_DECIMALS, ctx(test)),
        mint<USDC>(usdc_amount, USDC_DECIMALS, ctx(test)),
  ...
  public fun setup_5pool(
    ...
    next_tx(test, alice);
    {
      ...
      let (pool, lp_coin) = interest_clamm_stable::new_5_pool(
        &c,
        &coin_decimals,
        mint<DAI>(dai_amount, DAI_DECIMALS, ctx(test)),
        mint<USDC>(usdc_amount, USDC_DECIMALS, ctx(test)),
