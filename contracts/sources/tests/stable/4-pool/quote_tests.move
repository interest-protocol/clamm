...
#[test_only]
module clamm::stable_4pool_quote_tests {
  ...
  fun test_quote_add_liquidity() {
    ...
    next_tx(test, alice);
    {
      ...
      let expected_value = interest_clamm_stable::quote_add_liquidity<LP_COIN>(
       &mut pool, 
       &c, 
       vector[add_decimals(110, DAI_DECIMALS), add_decimals(120, USDC_DECIMALS), add_decimals(140, USDT_DECIMALS), add_decimals(150, FRAX_DECIMALS)]
      );
  ...
  fun test_quote_remove_liquidity_one_coin() {
    ...
    next_tx(test, alice);    
    {
      ...
      let (expected_value, _) = interest_clamm_stable::quote_remove_liquidity_one_coin<FRAX, LP_COIN>(&mut pool, &c, supply/ 10);
