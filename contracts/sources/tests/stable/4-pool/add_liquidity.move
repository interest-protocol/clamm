...
#[test_only]
module clamm::stable_tuple_4pool_add_liquidity_tests {
  ...
  fun add_liquidity() {
    ...
    next_tx(test, alice);
    {
      ...
      let expected_value = interest_clamm_stable::quote_add_liquidity<LP_COIN>(
        &mut pool, &c, 
        vector[
          add_decimals(100, DAI_DECIMALS), 
          add_decimals(110, USDC_DECIMALS), 
          add_decimals(120, USDT_DECIMALS),
          add_decimals(130, FRAX_DECIMALS)
        ]
      );
      ...
      let new_balances = vector[
        ...
        balances[3] + normalize_amount(130),
      ];
  ...
  fun add_liquidity_coins_wrong_order() {
    ...
    next_tx(test, alice);
    {
      ...
      burn(interest_clamm_stable::add_liquidity_4_pool<FRAX, USDC, DAI, USDT, LP_COIN>(
        &mut pool,
        &c,
        mint<FRAX>(130, FRAX_DECIMALS, ctx(test)),
        mint<USDC>(110, USDC_DECIMALS, ctx(test)),
  ...
  fun add_liquidity_coins_slippage() {
   let mut scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      burn(interest_clamm_stable::add_liquidity_4_pool<DAI, USDC, USDT, FRAX, LP_COIN>(
        &mut pool,
        &c,
        mint<DAI>(100, DAI_DECIMALS, ctx(test)),
        mint<USDC>(110, USDC_DECIMALS, ctx(test)),
