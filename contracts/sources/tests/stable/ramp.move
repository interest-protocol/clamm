...
module amm::stable_ramp_tests {
  ...
  fun ramp_logic() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      interest_stable::ramp<LP_COIN>(&mut pool, &admin_cap, &c, 500, future_a_time);

      let after_update_initial_a = interest_stable::initial_a<LP_COIN>(&pool);
      let after_update_initial_a_time = interest_stable::initial_a_time<LP_COIN>(&pool);

      let after_update_future_a = interest_stable::future_a<LP_COIN>(&pool);
      let after_update_future_a_time = interest_stable::future_a_time<LP_COIN>(&pool);
    ...
    next_tx(test, alice);
    {
      ...
      interest_stable::ramp<LP_COIN>(&mut pool,&admin_cap, &c, 250, (future_a_time as u256));
    ...
    next_tx(test, alice);
    {
      ...
      interest_stable::stop_ramp<LP_COIN>(&mut pool, &admin_cap, &c);
      ...
      assert_eq(amp, interest_stable::a<LP_COIN>(&pool, &c));
  ...
  #[test]
  #[expected_failure(abort_code = amm::errors::WAIT_ONE_DAY, location = amm::interest_stable)]
  fun ramp_abort_too_early() {
    ...
    next_tx(test, alice);
    {
      ...
      interest_stable::ramp<LP_COIN>(&mut pool,&admin_cap, &c, 1, 1);
  ...
  #[test]
  #[expected_failure(abort_code = amm::errors::FUTURE_TAMP_TIME_IS_TOO_SHORT, location = amm::interest_stable)]
  fun ramp_future_time_too_short() {
    ...
    next_tx(test, alice);
    {
      ...
      interest_stable::ramp<LP_COIN>(&mut pool, &admin_cap, &c, 1, ((MIN_RAMP_TIME * 2) as u256) );
  ...
  #[test]
  #[expected_failure(abort_code = amm::errors::INVALID_AMPLIFIER, location = amm::interest_stable)]
  fun ramp_abort_zero_a() {
    ...
    next_tx(test, alice);
    {
      ...
      // Ramp time is too short
      interest_stable::ramp<LP_COIN>(&mut pool, &admin_cap, &c, 0, ((MIN_RAMP_TIME * 2 + 1) as u256) );
  ...
  fun ramp_abort_a_too_high() {
    ...
    next_tx(test, alice);
    {
      ...
      // Ramp time is too short
      interest_stable::ramp<LP_COIN>(&mut pool, &admin_cap, &c, MAX_A, ((MIN_RAMP_TIME * 2 + 1) as u256) );
  ...
  fun ramp_up_too_high() {
    ...
    next_tx(test, alice);
    {
      ...
      // Ramp Up is too high
      interest_stable::ramp<LP_COIN>(&mut pool,&admin_cap,&c, 360 * MAX_A_CHANGE + 1, ((MIN_RAMP_TIME * 2 + 1) as u256) );
