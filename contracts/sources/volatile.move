...
module amm::volatile {
  ...
  public fun swap<CoinIn, CoinOut, LpCoin>(
    ...
    {  
      ...
      while ((state.n_coins as u64) > index) {
        ...
        *bal = mul_down(*bal, vector::borrow(&coin_states, index).price);
    ...
    let coin_out_amount = *vector::borrow(&balances_in_price, coin_out_state.index) - volatile_math::y(a, gamma, &balances_in_price, state.d, coin_out_state.index);
    ...
    if (needs_adjustment) {
      ...
      if (norm > math256::pow(adjustment_step, 2) && old_virtual_price != 0) {
        ...
        *x = d / state.n_coins;
        ...
        while ((state.n_coins as u64) > i) {
          let x = vector::borrow_mut(&mut xp, 0);
          *x = mul_down(d, state.n_coins * *vector::borrow(&p_new, i));
          i = i + 1;
        };
  ...
  public fun update_admin_fees<LpCoin>(pool: &mut InterestPool<Volatile>) {
    let (state, coin_states) = borrow_mut_state_and_coin_states<LpCoin>(pool);
    ...
    state.virtual_price = xcp_impl(state, coin_states, d) * ROLL / (balance::supply_value(&state.lp_coin_supply) as u256);
