...
module amm::stable_tuple_swap_tests {
 use std::option;
  ...
  use suitears::comparator::{Self, is_equal};
  ...
  fun swap_no_admin_fee() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      let coin_usdc = stable_tuple::swap<DAI, USDC, LP_COIN>(
        &mut pool,
        &c,
      ...
      assert_eq(is_equal(&comparator::compare(&balances_2, &expected_balances)), true);

      let (_, _, dai_balance) = stable_tuple::view_coin_state<DAI, LP_COIN>(&pool);
      let (_, _, usdc_balance) = stable_tuple::view_coin_state<USDC, LP_COIN>(&pool);
      let (_, _, usdt_balance) = stable_tuple::view_coin_state<USDT, LP_COIN>(&pool);
      ...
      assert_eq(is_equal(&comparator::compare(&expected_balances, &coin_balances)), true);
  ...
  fun swap_admin_fee() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(stable_tuple::view_admin_balance<DAI, LP_COIN>(&pool), (admin_fee_in as u64));
