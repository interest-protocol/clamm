  ...
  fun sets_2pool_state_correctly() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_clamm_volatile::lp_coin_supply<LP_COIN>(&pool), 387298334620);
      assert_eq(interest_clamm_volatile::balances<LP_COIN>(&pool), vector[normalize_amount(15000), normalize_amount(10)]);
      ...
      assert_eq(interest_clamm_volatile::balances_in_price<LP_COIN>(&pool), vector[normalize_amount(15000), 10 * INITIAL_ETH_PRICE]);
