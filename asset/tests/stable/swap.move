  ...
  fun swap_no_admin_fee() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      let dai_balance = interest_clamm_stable::coin_balance<DAI, LP_COIN>(&pool);
      let usdc_balance = interest_clamm_stable::coin_balance<USDC, LP_COIN>(&pool);
      let usdt_balance = interest_clamm_stable::coin_balance<USDT, LP_COIN>(&pool);
  ...
  fun swap_admin_fee() {
   let scenario = scenario();
    ...
    next_tx(test, alice);
    {
      ...
      assert_eq(interest_clamm_stable::admin_balance<DAI, LP_COIN>(&pool), (admin_fee_in as u64));
      assert_eq(interest_clamm_stable::admin_balance<USDC, LP_COIN>(&pool), admin_fee_out);
